import React from "react";
import { useDispatch, useSelector } from "react-redux";
import { Link } from "react-router-dom";
import { setIsLogin, setUserData } from "../redux/reducer/userReducer";


const Header = () => {

    let cartItemCount = useSelector((state: any) => state.cart.cartItemCount);
    const userData = useSelector((state: any) => state.user.userData);
    const isLogin = useSelector((state: any) => state.user.isLogin);
    const dispatch = useDispatch();

    const { name, img } = userData.name !== "" ? userData : { name: "Guest", img: "data:image/png;base64," };

    const logout = () => {
        dispatch(setUserData({
            name: "",
            img: "",
            password: "",
            email: ""
        }));
        dispatch(setIsLogin(false));

    }

    return <>
        <div>
            <nav className="navbar navbar-light bg-light">
                <div className="container-fluid">
                    <Link to={"/"}><button className="navbar-brand">Shop Cruse</button></Link>
                    <h4 className="navbar-brand">{name} <img src={img} alt="profile-image" style={{ width: "2.5rem", borderRadius: "50%" }} /></h4>
                    {isLogin && <button className="btn btn-danger" onClick={logout}>Logout</button>}
                    <div className="d-flex">
                        <Link to={"/cart"}>
                            <button className="btn btn-outline-success position-relative" type="submit"> 🛒 Cart {cartItemCount > 0 ? <span className="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" >{cartItemCount}</span> : null}</button>
                        </Link>
                    </div>
                </div>
            </nav>
        </div>
    </>
}


export default Header;